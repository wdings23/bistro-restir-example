    ...
    @builtin(workgroup_id) workGroup: vec3<u32>)
{
    ...
    //aDebug[iMesh].mXFormMin = xformMin;
    //aDebug[iMesh].mXFormMax = xformMax;

    //aDebug[iMesh].miNumIndices = u32(iNumIndices);
    //aDebug[iMesh].miIndexAddressOffset = u32(iIndexAddressOffset);
    //aDebug[iMesh].miIndirectDrawIndex = iDrawCommandIndex;
    //aDebug[iMesh].miDraw = u32(bInside);

    atomicAdd(&aNumDrawCalls[1], 1u);
    //if(aNumDrawCalls[1] >= uniformData.miNumMeshes)
    //{
    //    sortDrawCalls();
    //}
    ...
    iMesh: u32) -> bool
{
    ...
    let diffXY: vec2<f32> = abs(maxXYZ.xy - minXYZ.xy);
    ...
    uv.y = 1.0f - uv.y;
    uv = clamp(uv, vec2<f32>(0.0f, 0.0f), vec2<f32>(1.0f, 1.0f));
