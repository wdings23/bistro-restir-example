...
fn fs_main(in: VertexOutput) -> FragmentOutput 
{
    ...
    for(var iSlice: i32 = 0; iSlice < kiNumSlices; iSlice++)
    {
        ...
        for(var iSector: i32 = -kiNumSectors / 2; iSector < kiNumSectors / 2; iSector++)
        {
            ...
            let currRadiance: vec3<f32> = 
                sampleRadiance.xyz *
                (f32(CountBits(iSampleBitMask & (~iBitMask))) / f32(kiNumSectors)) * 
                sampleNormalDP *
                origNormalDP;
            let fAttenuation: f32 = 1.0f / (fWorldDiffLength * fWorldDiffLength);
            totalRadiance += currRadiance * fAttenuation;
    ...
    //totalRadiance /= f32(kiNumSlices);
    out.radiance = vec4<f32>(totalRadiance.xyz, 1.0f);
